...
public class DamageAction extends AbstractAmmunitionAction {
    ...
    public void fireAction(Player player, GunshellRayTraceResult rayTraceResult, ConfigurationSection configuration) {
        ...
        if (PluginUtils.getInstance().isGunDamageDisabledInWorld(player.getWorld().getName())
                || (livingEntity.getWorld() != null
                && PluginUtils.getInstance().isGunDamageDisabledInWorld(livingEntity.getWorld().getName()))) return;
        ...
        if (compatibilityManager.isExtensionEnabled(GriefPreventionExtension.class)
                ...
                .canUseWeapons(player, livingEntity.getLocation())) return;
