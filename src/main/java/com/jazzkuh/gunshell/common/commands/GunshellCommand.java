...
public class GunshellCommand extends ExtendedAnnotationCommand {
    ...
    @Usage("<weaponType> [player]")
    @Permission("gunshell.commands.gunshell.getweapon")
    public void onGetWeapon(CommandSender sender, GunshellFireable fireable, @Optional Player player) {
        if (player == null) {
        ...
        player.getInventory().addItem(fireable.getItemStack());
        MessagesConfig.SUCCESSFULLY_ADDED_TO_INVENTORY.get(sender);
    }
    ...
    @Subcommand("getmelee")
    @Usage("<meleeType> [player]")
    @Permission("gunshell.commands.gunshell.getmelee")
    public void onGetMelee(CommandSender sender, GunshellMelee melee, @Optional Player player) {
        if (player == null) {
            if (!(sender instanceof Player)) {
                MessagesConfig.ERROR_PLAYER_NOT_FOUND.get(sender);
                return;
            ...
            ((Player) sender).getInventory().addItem(melee.getItemStack());
            MessagesConfig.SUCCESSFULLY_ADDED_MELEE_TO_INVENTORY.get(sender);
            return;
        }

        player.getInventory().addItem(melee.getItemStack());
        MessagesConfig.SUCCESSFULLY_ADDED_MELEE_TO_INVENTORY.get(sender);
    }
}
