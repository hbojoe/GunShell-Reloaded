...
public class PluginUtils {
    ...
    public boolean isGunUseDisabledInWorld(String worldName) {
        return this.isWorldListed(DefaultConfig.DISABLED_GUN_USE_WORLDS.asList(), worldName);
    }

    public boolean isGunDamageDisabledInWorld(String worldName) {
        return this.isWorldListed(DefaultConfig.DISABLED_GUN_DAMAGE_WORLDS.asList(), worldName);
    }

    private boolean isWorldListed(List<String> worlds, String worldName) {
        if (worldName == null || worlds == null || worlds.isEmpty()) return false;
        return worlds.stream().anyMatch(world -> world != null && world.equalsIgnoreCase(worldName));
    }
}
